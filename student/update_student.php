...
$hosting = "http://192.168.1.120/wri/";
$target_file = $target_dir . time() . "-" . basename($_FILES["image"]["name"]);
$uploadOK = 1;
$imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));
$check  = getimagesize($_FILES["image"]["tmp_name"]);
$thumbnailstudent = $hosting . "images/student/" . time() . "-" . basename($_FILES["image"]["name"]);
$emailUser = $_POST["emailUser"];
$nameStudent = $_POST["nameStudent"];
$birthdayStudent = $_POST["birthdayStudent"];
$major = $_POST["major"];
$phoneUser = $_POST["phoneUser"];
$querryupStu = "UPDATE `students` SET `name_stu`='$nameStudent',`thumbnail_stu`='$thumbnailstudent',`birthday_stu`='$birthdayStudent',`major`='$major' WHERE `email_user` = '$emailUser'";
